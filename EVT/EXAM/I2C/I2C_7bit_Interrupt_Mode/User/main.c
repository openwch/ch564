...
void I2C_EV_IRQHandler(void) __attribute__((interrupt("WCH-Interrupt-fast")));
void I2C_ER_IRQHandler(void) __attribute__((interrupt("WCH-Interrupt-fast")));
...
void IIC_Init(u32 bound, u16 address)
{
    ...
    NVIC_EnableIRQ(IIC_EV_IRQn);
    NVIC_EnableIRQ(IIC_ER_IRQn);

    NVIC_SetPriority(IIC_EV_IRQn, 0);
    NVIC_SetPriority(IIC_ER_IRQn, 0);
...
void I2C_EV_IRQHandler(void)
{
    ...
    if (I2C_GetITStatus(I2C, I2C_IT_SB) != RESET)
    {
        ...
        {
            master_sate = 4;
            I2C_Send7bitAddress(I2C, 0x02, I2C_Direction_Receiver);
        }
    }
    ...
    else if (I2C_GetITStatus(I2C, I2C_IT_ADDR) != RESET)
    {
        ...
        ((void)I2C_ReadRegister(I2C, I2C_Register_STAR2));
    }
    else if (I2C_GetITStatus(I2C, I2C_IT_TXE) != RESET)
    {
        if (master_sate == 2)
        {
            ...
            {
                I2C_SendData(I2C, TxData[master_send_len]);
                master_send_len++;
            }
            else
            {
                master_sate = 3;
                I2C_GenerateSTART(I2C, ENABLE);
                I2C_SendData(I2C, 0xff); // dummy byte, to prevent next TxE
            }
        }
    }
    else if (I2C_GetITStatus(I2C, I2C_IT_RXNE) != RESET)
    {
        ...
        if (master_sate == 5)
        {
            ...
            {
                RxData[master_recv_len] = I2C_ReceiveData(I2C);
                master_recv_len++;
                if (master_recv_len == 5)
                {
                    I2C_NACKPositionConfig(I2C, I2C_NACKPosition_Next); // clear ack
...
#elif (I2C_MODE == SLAVE_MODE)
    if (I2C_GetITStatus(I2C, I2C_IT_ADDR) != RESET)
    {
        if (I2C_GetFlagStatus(I2C, I2C_FLAG_TRA) && I2C_GetFlagStatus(I2C, I2C_FLAG_TXE))
        {
            // write mode
        }
        else
    ...
    {
        RxData[slave_recv_len] = I2C_ReceiveData(I2C);
        slave_recv_len++;
    }
    else if (I2C_GetITStatus(I2C, I2C_IT_STOPF) != RESET)
    {
        I2C->CTLR1 &= I2C->CTLR1;
        ((void)(I2C->STAR1));
    }
    ...
    {
        ((void)I2C_ReadRegister(I2C, I2C_Register_STAR1));
        ((void)I2C_ReceiveData(I2C));
    }
    else if (I2C_GetITStatus(I2C, I2C_IT_SB) != RESET)
    {
        ((void)I2C_ReadRegister(I2C, I2C_Register_STAR1));
        ((void)I2C_ReceiveData(I2C));
    }
    ...
    {
        I2C_SendData(I2C, RxData[slave_send_len]);
        slave_send_len++;
    }
#endif
    else
...
#elif (I2C_MODE == SLAVE_MODE)
    if (I2C_GetITStatus(I2C, I2C_IT_AF))
    {
