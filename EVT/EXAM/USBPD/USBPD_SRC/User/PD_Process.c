...
void PD_Init( void )
{
    GPIOB_ModeCfg(GPIO_Pin_18, GPIO_ModeIN_Floating);   //CC1
    GPIOB_ModeCfg(GPIO_Pin_19, GPIO_ModeIN_Floating);   //CC2
    EXTEN->CTLR2|=RB_USBPD_IN_HVT0;
    EXTEN->CTLR2|=RB_USBPD_IN_HVT1;
...
UINT8 PD_Detect( void )
{
    ...
    if(PD_Ctl.Flag.Bit.Connected)                                       /*Detect disconnection*/
    {
        ...
        if((GPIOB->PIN & PIN_CC1) != (uint32_t)Bit_RESET)
        {
            cmp_cc1 |= bCC_CMP_220;
        }
        if((GPIOB->PIN & PIN_CC2) != (uint32_t)Bit_RESET)
        {
            cmp_cc2 |= bCC_CMP_220;
        }
...
void PD_Main_Proc( )
{
    ...
    switch( PD_Ctl.PD_State )
    {
        case STA_DISCONNECT:
            ...
            GPIOB_ITModeCfg(GPIO_Pin_18, GPIO_ITMode_LowLevel);
            GPIOB_ITModeCfg(GPIO_Pin_19, GPIO_ITMode_LowLevel);
            RCC_SlpWakeCtrl(RB_SLP_PB_WAKE, ENABLE);
