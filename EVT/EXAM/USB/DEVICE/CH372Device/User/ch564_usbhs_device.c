...
void USBHS_Device_Endp_Init ( void )
{
    ...
    USBHSD->UEP_TX_EN = RB_EP0_EN | RB_EP1_EN | RB_EP4_EN | RB_EP6_EN;
    USBHSD->UEP_RX_EN = RB_EP0_EN | RB_EP1_EN | RB_EP3_EN | RB_EP5_EN;

    USBHSD->UEP0_MAX_LEN = DEF_USBD_UEP0_SIZE;
    USBHSD->UEP1_MAX_LEN = DEF_USB_EP1_HS_SIZE;
    USBHSD->UEP2_MAX_LEN = DEF_USB_EP2_HS_SIZE;
    USBHSD->UEP3_MAX_LEN = DEF_USB_EP2_HS_SIZE;
    USBHSD->UEP4_MAX_LEN = DEF_USB_EP2_HS_SIZE;
    USBHSD->UEP5_MAX_LEN = DEF_USB_EP2_HS_SIZE;
    USBHSD->UEP6_MAX_LEN = DEF_USB_EP2_HS_SIZE;
    ...
    if( intflag & RTX_ACT_IF )
    {
        ...
        if( !(USBHSD->INT_ST & RB_UIS_EP_DIR )) // SETUP/OUT Transaction
        {
            switch( endp_num )
            {
                ...
                case  DEF_UEP1:
                    if ( USBHSD->UEP1_RX_CTRL & USBHS_ENDP_R_TOG_MATCH )
                    {
                        /* Write In Buffer */
                ...
                case  DEF_UEP3:
                    if ( USBHSD->UEP3_RX_CTRL & USBHS_ENDP_R_TOG_MATCH )
                    {
                    ...
                    USBHSD->UEP3_RX_CTRL &= ~USBHS_UEP_ENDP_R_DONE;
                   break;

                /* end-point 5 data out interrupt */
                case  DEF_UEP5:
                if ( USBHSD->UEP5_RX_CTRL & USBHS_ENDP_R_TOG_MATCH )
                {
          ...
          /* data-in stage processing */
            switch ( endp_num )
            {
                ...
                case DEF_UEP6:
                    ...
                    USBHSD->UEP6_TX_CTRL &= ~USBHS_UEP_ENDP_T_DONE;
                    break;
