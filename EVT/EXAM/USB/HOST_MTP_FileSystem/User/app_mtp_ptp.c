...
uint8_t MTP_SendData( uint8_t usb_port, uint8_t *pbuf, uint32_t len )
{
    ...
    while( len )
    {
        if( usb_port == DEF_USBFS_PORT_INDEX )
        {
            ...
            s = USBHSH_Transact( USB_PID_OUT  | (HostCtl[ index ].Interface[ 0 ].OutEndpAddr[ 0 ] << 4 ),
                                 HostCtl[ index ].Interface[ 0 ].OutEndpTog[ 0 ], 20000 );
            if( s == ERR_SUCCESS )
            {
                HostCtl[ index ].Interface[ 0 ].OutEndpTog[ 0 ] ^= TOG_DATA1;
...
uint8_t MTP_RecvData( uint8_t usb_port, uint8_t *pbuf, uint32_t *plen )
{
    ...
    if( usb_port == DEF_USBFS_PORT_INDEX )
    {
        s = USBHSH_Transact( USB_PID_IN  | (HostCtl[ index ].Interface[ 0 ].OutEndpAddr[ 0 ] << 4 ),
                             HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ], 20000 );

        if( s == ERR_SUCCESS )
        {
            HostCtl[ index ].Interface[ 0 ].InEndpTog[ 0 ] ^= TOG_DATA1;
